class MusicLibraryController
    ...
    def list_songs_by_artist
        ...
        if Artist.find_by_name(artist_name)
            chosen_artist = Artist.find_by_name(artist_name)
            sorted_songs = chosen_artist.songs.sort_by {|song| song.name}
    ...
    def list_songs_by_genre
        ...
        if Genre.find_by_name(genre_name)
            chosen_genre = Genre.find_by_name(genre_name)
            sorted_songs = chosen_genre.songs.sort_by {|song| song.name}
            sorted_songs.each_with_index do |song, index|
    ...
    def play_song
        ...
        if (1..Song.all.length).include?(number)
